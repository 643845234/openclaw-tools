...
━━━ Cycle ${this.cycleCount} | ${now} ━━━` });
    ...
    for (const symbol of Object.keys(this.paperEngine.positions)) {
      ...
      const side = result.trade && result.trade.side ? result.trade.side : (this.paperEngine.positions[symbol] && this.paperEngine.positions[symbol].side);
      const leverage = result.trade && typeof result.trade.leverage === 'number' ? result.trade.leverage : undefined;
    ...
    const actionable = opportunities.filter(opp => {
      ...
      if (opp.signal === 'SELL') return shortEnabled ? true : Boolean(this.paperEngine.positions[opp.symbol]);
      return false;
    });
    ...
    for (const opp of actionable.slice(0, 3)) {
      ...
      if (opp.signal === 'BUY' && !this.paperEngine.positions[opp.symbol]) {
        const leverage = chooseLeverage(opp.score);
        ...
        if (this.paperEngine.positions[opp.symbol]) {
          ...
          if (result.success) {
            agentMessage('log', { message: `💰 CLOSE ${opp.symbol} @ $${currentPrice.toFixed(6)} | P/L: ${result.trade.profitPercent?.toFixed(2)}%` });
            const payload = {
              action: 'CLOSE',
              symbol: opp.symbol,
              price: currentPrice,
          ...
          const leverage = chooseLeverage(opp.score);
          const result = this.paperEngine.open(
            opp.symbol,
            'SHORT',
            currentPrice,
            Date.now(),
          ...
          if (result.success) {
            agentMessage('log', { message: `✅ SHORT ${opp.symbol} @ $${currentPrice.toFixed(6)} | Strategy: ${opp.strategy} | Consensus: ${opp.consensus}` });
            const payload = {
              action: 'OPEN',
              symbol: opp.symbol,
              price: currentPrice,
...
Open Positions:` });
  ...
  async run() {
    ...
    try {
      ...
      const learned = await StrategyOptimizer.loadStrategyWeights(config);
      const base = (this.tunedConfig && this.tunedConfig.baseStrategyWeights) ? this.tunedConfig.baseStrategyWeights : config.baseStrategyWeights;
      const blended = { ...(base || {}) };
      for (const [name, w] of Object.entries(learned || {})) {
        const baseW = typeof blended[name] === 'number' ? blended[name] : 1;
        blended[name] = baseW * w;
      }
      this.strategyWeights = blended;
